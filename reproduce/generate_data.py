...
def load_deepmirtar(pos_file, neg_file):
    ...
    all_deepmirtar = pd.concat((pos, neg)).drop_duplicates(
        ['miRNA_seq', 'site_seq']).reset_index(drop=True)

    all_deepmirtar['miRNA_seq'] = all_deepmirtar['miRNA_seq'].map(
        rename_mirseq)
    ...
    all_deepmirtar['mRNA Seq'] = all_deepmirtar['mRNA Seq'].map(
        rename_utrseq).map(lambda x: x[::-1])
    # get site 3' -> 5' sequence
    all_deepmirtar['site_seq'] = all_deepmirtar['site_seq'].map(
        rename_utrseq).map(lambda x: x[::-1])
    ...
    all_deepmirtar = all_deepmirtar.sort_values(
        by=['miR_ID']).reset_index(drop=True)
...
def load_mirtarbase(pos_file, v22_miRNA2seq):
    ...
    mi2site = mi2site.rename(
        columns={'Target Site': 'site_seq', 'miRNA': 'miR_ID'})
    ...
    mi2site = mi2site.loc[(mi2site['site_seq_len'] <= 50)
                          & (mi2site['miRNA_seq_len'] <= 30)]
...
def filter_pos(all_pos):
    ...
    subprocess.call(["miranda", "./tmp/all_mir.fa", "./tmp/all_site.fa", "-sc", "100",
                     "-restrict", "./tmp/interaction_pair.txt", "-out", "./tmp/miranda_result.txt"])
...
def p_args():
    ...
    parser.add_argument(
        "deepmirtar_pos", help="deepmirtar positive interaction pairs data  (source: https://academic.oup.com/bioinformatics/article/34/22/3781/5026656#supplementary-data)")
    parser.add_argument(
        "deepmirtar_neg", help="deepmirtar negative interaction mock pairs data  (source: https://academic.oup.com/bioinformatics/article/34/22/3781/5026656#supplementary-data)")
    parser.add_argument(
        "mirtarbase_pos", help="mirtarbase positive interaction pair data  (source: http://mirtarbase.cuhk.edu.cn/php/download.php)")
    parser.add_argument(
        "v22_mirna", help="mirBase v22 mature microRNA file (source: http://www.mirbase.org/ftp.shtml)")
    parser.add_argument(
        "output_file", help="output whole dataset in pickle format after adding mock data")
    parser.add_argument("--mock_threshold", default=2000,
                        help="maximun shuffle times for mock data generation")
    parser.add_argument("--filter", default=False, type=bool,
                        help="determine if the positive data filted by miranda score 100")
    parser.add_argument("--seed", default=9487, type=bool,
                        help="seed for fisher shuffle in order to reproduce same mock data")
    parser.add_argument("--only_deepmirtar", default=False,
                        type=bool, help="generate only deepmirtar data")
...
def main():
    ...
    if args.only_deepmirtar:
        ...
        all_pos = pd.concat(
            (pos_mirtarbase, all_deepmirtar.loc[all_deepmirtar['label'] == 1]))
        logging.info('total positive data before filter: %s from deepmirtar %d from miRTarBase %d' % (
            str(len(all_pos)), len(all_deepmirtar.loc[all_deepmirtar['label'] == 1]), len(pos_mirtarbase)))

        logging.info('filtering data with miRanda')
        if args.filter == True:
            all_pos = filter_pos(all_pos)
            logging.info(
                'total positive data after filtering: '+str(len(all_pos)))

        random.seed(args.seed)
        logging.info('generating mock data')
        mock_mir = gen_mock_mir(
            all_pos, seed_27, seed_38, int(args.mock_threshold))
        all_neg = gen_mock_data(all_pos, mock_mir)

        all_data = pd.concat([all_pos, all_neg]).reset_index(drop=True)
        all_data = all_data.loc[(all_data['site_idx']+all_data['mir_idx']).apply(
            tuple, 1).drop_duplicates().index].reset_index(drop=True)
